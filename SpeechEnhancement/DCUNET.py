...
class GrandChallengeDataset_Test(torch.utils.data.Dataset):

    def __init__(self, data_test):
        self.wav_list = data_test
        test_wav, _ = librosa.load(self.wav_list, sr=16000)  # mono로 읽음
        test_wav = torch.from_numpy(test_wav).unsqueeze(0)
        self.tgt_wav_len = test_wav.shape[1]
        ...
        self.test_spec = torch.stft(test_wav,
                                                           hop_length=self.hop_len, n_fft=self.win_len,
                                                           )[0]
...
if __name__ == "__main__":
    list_target = glob.glob(os.path.join("/home/nas/DB/AIG2022/VAD_sample/","*.wav"))
    print(len(list_target))

    enhance(list_target,"/home/nas/DB/AIG2022/VAD_sample/out/","Unet_V2.pth")